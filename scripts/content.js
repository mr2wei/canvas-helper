...
mainExtension = () => {
  ...
  chrome.storage.local.get(
    ["classes", "classesHash", "plannable", "plannableHash"],
    (data) => {
      if (chrome.runtime.lastError) {
        console.error(
            ...
            .then((classes) => {
              ...
              hashJson(classesString).then((hash) => {
                chrome.storage.local.set(
                  { classes: classes, classesHash: hash },
                  () => {
                    if (chrome.runtime.lastError) {
                      console.error(
                        "Error setting classes and classesHash:",
                        chrome.runtime.lastError
                      );
                    } else {
                      console.log(
                        "Classes and classesHash saved successfully."
                      );
                    }
                  }
                );
          ...
          fetchPlannable()
            .then((plannable) => {
              ...
              hashJson(plannableString).then((hash) => {
                chrome.storage.local.set(
                  { plannable: plannable, plannableHash: hash },
                  () => {
                    if (chrome.runtime.lastError) {
                      console.error(
                        "Error setting plannable and plannableHash:",
                        chrome.runtime.lastError
                      );
                    } else {
                      console.log(
                        "Plannable and plannableHash saved successfully."
                      );
                    }
                  }
                );
...
setCanvasDomain = async () => {
  ...
  if (classes.errors) {
    ...
    console.log(
      "Received an unexpected response, possibly not a Canvas domain."
    );
  } else {
...
checkClasses = async (storedClassesHash) => {
  ...
  if (newHash !== storedClassesHash) {
    ...
    chrome.storage.local.set(
      { classes: newClasses, classesHash: newHash },
      () => {
        if (chrome.runtime.lastError) {
          console.error(
            "Error setting classes and classesHash:",
...
checkPlannable = async (storedPlannableHash) => {
  ...
  if (newHash !== storedPlannableHash) {
    ...
    chrome.storage.local.set(
      { plannable: newPlannable, plannableHash: newHash },
      () => {
        if (chrome.runtime.lastError) {
          console.error(
            "Error setting plannable and plannableHash:",
...
fetchPlannable = async () => {
  try {
    ...
    const plannable = await fetchData(
      domain +
    ...
    (async () => {
      try {
        ...
        await new Promise((resolve, reject) => {
          chrome.storage.local.set(
            { classes: classes, classesHash: classesHash },
            () => {
              if (chrome.runtime.lastError) {
                console.error(
                  "Error setting classes and classesHash:",
        ...
        await new Promise((resolve, reject) => {
          chrome.storage.local.set(
            { plannable: plannable, plannableHash: plannableHash },
            () => {
              if (chrome.runtime.lastError) {
                console.error(
                  "Error setting plannable and plannableHash:",
